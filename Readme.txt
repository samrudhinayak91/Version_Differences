...
The MainTest class is the JUnit class for unit testing and integration testing. It has 3 test cases, one for each of the function. The path to the test database has been hardcoded to the absolute path in my folder on my system. This needs to be updated to  reflect the absolute path in the system on which the project is run in order to run the test cases successfully.
