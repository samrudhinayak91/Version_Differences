...
The MainTest class is the JUnit class for unit testing. It has 3 test cases, one for each of the function. The class path for testing is hardcoded to give the absolute path from my current folder. This needs to be updated to reflect the absolute path on the machine on which it is run in order to run the test cases successfully.
...
This will run the junit class to do the comparison.
